...
| V1.3 | 2026-08-27 | 结合 W1–W3 数据执行中的锁定前发现更正：① COVID 探索队列因 MIMIC-IV-ECG v1.0 波形采集不覆盖 2020–2022 时段而不成立（2.4、第六、七、9.6 节相应修订）；② 5.1 节 Track A 输入规格更正为 Lead II 10 s@250 Hz（匹配 V14 编码器原生输入，冻结迁移须与训练分布一致）；③ ECG 信号质控阈值数值写入 3.4 节并冻结；④ 第六章队列与事件数预期按实际提取结果回填 | — |
...
- 开发队列：MIMIC-IV（2008–2016）；时间外推验证：MIMIC-IV（2017–2019）。
- ~~2020–2022（COVID 时段）仅作探索。~~**（V1.3 修订）COVID 探索队列不成立**：数据提取证实 MIMIC-IV-ECG v1.0 波形采集不覆盖 2020–2022 时段（该时段住院患者 ECG 覆盖率仅约 3.5%，本队列 3,335 例 COVID 时段患者 t0±24 h 内无一份可链接 ECG），相关预设探索分析取消。
...
- **质控**：自动质控规则预设于代码仓库；质控失败者计入流程图排除项，不做人工挽救。**（V1.3 冻结）信号质量阈值**：电极饱和平台（导联取值停留于本导联极值的最长连续段）>100 ms；基线漂移（0.6 s 中值滤波基线 p95–p05 极差）>1.0 mV；高频噪声（>40 Hz 四阶 Butterworth 高通分量 RMS）>0.05 mV；阈值定义与实现见代码 `src/qc_config.yaml` 与 `src/ecg_link.py`。
...
| 轨道 | 输入 | 编码器 | 输出 | 角色 |
|---|---|---|---|---|
| **Track A（主）** | Lead II 10 s 片段，重采样至 250 Hz 共 2500 点（V14 编码器原生输入规格；V1.3 由"8 s"更正） | V14 一维 CNN 编码器，**冻结迁移**（权重不在本队列重训） | 16 维潜向量 z1–z16 | 主要分析 |
...
- **预期样本**：Sepsis-3 队列且 ECG 链接成功者预计 1.5 万–2.5 万例；28 天死亡事件数预计 2500–4500 例。**（V1.3 回填）实际：最终分析队列 16,499 例（开发 14,780 / 时间外推 1,719），28 天死亡 3,002 例（18.2%），均落在预计区间内。**
...
- 时间外推队列（2017–2019）事件数预计 ≥800，满足外部验证事件数 ≥100 的经验下限。**（V1.3 修正）实际时间外推队列 1,719 例、28 天死亡约 380 例（事件率 22.1%）：高于 ≥100 的经验下限，但低于原预计 ≥800；第六章 ΔAUC 功效核算与 H3 检验均须以实际样本量与事件数为准，结论表述相应谨慎。**
...
| ~~2020–2022~~ | ~~COVID 探索队列~~ | **（V1.3）不成立**：MIMIC-IV-ECG v1.0 不覆盖该时段，无可用 ECG，预设探索分析取消（见 2.4 节） |
...
- ~~COVID 队列仅探索性报告 AUC 与校准曲线。~~（V1.3 删除：COVID 队列不成立，见 2.4 节）
